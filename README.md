...
`/apply` opens the daily reels service form. Price links use `service=reels` and preselect `plan=Standard|Deluxe|Premium`. The service adds a setup supply amount of 100,000 KRW; monthly supply amounts are Standard 149,000 / Deluxe 599,000 / Premium 999,000 KRW, with 10% VAT. Exact first-month VAT-inclusive totals are 273,900 / 768,900 / 1,208,900 KRW. The single **결제하기** button currently submits an intake request (not a payment) as JSON to the existing Railway `customer-gateway` at `https://customer-gateway-staging.up.railway.app/api/applications`. The server stores a separate **pending-review** application and returns a random receipt. **Payment processing remains unconnected**, clearly disclosed beside the button; submission never registers a customer, authorizes paid production, starts a worker, posts content or sends bot messages. Prices are controlled by the server, not the browser.
